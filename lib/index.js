...
async function run() {
    try {
        ...
        let howtestExists = utils_1.default.checkSectionModified('How to test', prBody, "### How to test", "### Affected Features & Quality Assurance Scope");
        if (!howtestExists) {
            core.setFailed(`How to test not set."`);
            return;
        }
        let impactedScopeExists = utils_1.default.checkSectionModified('Affected Features & Quality Assurance Scope', prBody, "### Affected Features & Quality Assurance Scope", '## Technical description');
